...
class UserPreferencesRepository @Inject constructor(
    ...
    companion object {
        ...
        val oneRepMaxFormulaKey = intPreferencesKey("one_rep_max_formula")
        val sleepModeKey = booleanPreferencesKey("sleep_mode")
        val showRpeKey = booleanPreferencesKey("show_rpe")
        val intensityScaleKey = intPreferencesKey("intensity_scale")
        val restTimerVibrationKey = booleanPreferencesKey("rest_timer_vibration")
    }
    ...
    val oneRepMaxFormula: StateFlow<OneRepMaxFormula> = dataStore.data
        .map { preferences ->
            OneRepMaxFormula.entries.find { it.value == preferences[oneRepMaxFormulaKey] }
                ?: OneRepMaxFormula.BALANCED
        }
        .stateIn(
            scope = applicationScope,
            started = SharingStarted.Eagerly,
            initialValue = OneRepMaxFormula.BALANCED
        )

    val sleepModeEnabled: StateFlow<Boolean> = dataStore.data
        .map { preferences -> preferences[sleepModeKey] == true }
        .stateIn(
            scope = applicationScope,
            started = SharingStarted.Eagerly,
            initialValue = false
    ...
    val showRpe: StateFlow<Boolean> = dataStore.data
        .map { preferences -> preferences[showRpeKey] == true }
        .stateIn(
            scope = applicationScope,
            started = SharingStarted.Eagerly,
            initialValue = false
    ...
    val intensityScale: StateFlow<org.nexc.core.enums.userPreferences.IntensityScale> = dataStore.data
        .map { preferences ->
            org.nexc.core.enums.userPreferences.IntensityScale.entries.find { it.value == preferences[intensityScaleKey] }
                ?: org.nexc.core.enums.userPreferences.IntensityScale.RPE
        }
        .stateIn(
            scope = applicationScope,
            started = SharingStarted.Eagerly,
            initialValue = org.nexc.core.enums.userPreferences.IntensityScale.RPE
        )

    val restTimerVibrationOn: StateFlow<Boolean> = dataStore.data
        .map { preferences -> preferences[restTimerVibrationKey] != false }
        .stateIn(
            scope = applicationScope,
            started = SharingStarted.Eagerly,
            initialValue = true
